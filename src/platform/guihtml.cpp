...
public:
    ...
    double ThawFloat(const std::string &key, double defaultValue = 0.0) {
        ...
        if(value == val::null())
        return defaultValue;
        return val::global("parseFloat")(value).as<float>();
    }
...
class FileOpenDialogImplHtml_Obsolute : public FileDialog {
public:
    std::string title;
    std::string filename;
    std::string filters;
    ...
    FileOpenDialogImplHtml_Obsolute() {
        //FIXME(emscripten):
        dbp("FileOpenDialogImplHtml_Obsolute::FileOpenDialogImplHtml_Obsolute()");
        ...
        dbp("FileOpenDialogImplHtml_Obsolute::FileOpenDialogImplHtml_Obsolute() OK.");
    }

    ~FileOpenDialogImplHtml_Obsolute() override {
        dbp("FileOpenDialogImplHtml_Obsolute::~FileOpenDialogImplHtml_Obsolute()");
    ...
    void SetTitle(std::string title) override {
        //FIXME(emscripten):
        dbp("FileOpenDialogImplHtml_Obsolute::SetTitle(): title=%s", title.c_str());
        this->title = title;
        //FIXME(emscripten):
    ...
    void SetCurrentName(std::string name) override {
        //FIXME(emscripten):
        dbp("FileOpenDialogImplHtml_Obsolute::SetCurrentName(): name=%s", name.c_str());
        SetFilename(GetFilename().Parent().Join(name));
    }

    Platform::Path GetFilename() override {
        //FIXME(emscripten):
        dbp("FileOpenDialogImplHtml_Obsolute::GetFilename()");
        return Platform::Path::From(this->filename.c_str());
    }

    void SetFilename(Platform::Path path) override {
        //FIXME(emscripten):
        dbp("FileOpenDialogImplHtml_Obsolute::SetFilename(): path=%s", path.raw.c_str());
    ...
    void SuggestFilename(Platform::Path path) override {
        //FIXME(emscripten):
        dbp("FileOpenDialogImplHtml_Obsolute::SuggestFilename(): path=%s", path.raw.c_str());
        SetFilename(Platform::Path::From(path.FileStem()));
    }

    void AddFilter(std::string name, std::vector<std::string> extensions) override {
        //FIXME(emscripten):
        dbp("FileOpenDialogImplHtml_Obsolute::AddFilter()");
        this->filters = "";
    ...
    void FreezeChoices(SettingsRef settings, const std::string &key) override {
        //FIXME(emscripten):
        dbp("FileOpenDialogImplHtml_Obsolute::FreezeChoise()");
    }

    void ThawChoices(SettingsRef settings, const std::string &key) override {
        //FIXME(emscripten):
        dbp("FileOpenDialogImplHtml_Obsolute::ThawChoices()");
    }

    bool RunModal() override {
        //FIXME(emscripten):
        dbp("FileOpenDialogImplHtml_Obsolute::RunModal()");
        this->filename = "/untitled.slvs";
        this->fileUploadHelper.call<void>("showDialog");

        //FIXME(emscripten): use val::await() with JavaScript's Promise
        dbp("FileOpenDialogImplHtml_Obsolute: start loop");
        // Wait until fileUploadHelper.is_shown == false
        while (true) {
            bool is_shown = this->fileUploadHelper["is_shown"].as<bool>();
            if (!is_shown) {
                // dbp("FileOpenDialogImplHtml_Obsolute: break due to is_shown == false");
                break;
            } else {
                // dbp("FileOpenDialogImplHtml_Obsolute: sleep 100msec... (%d)", is_shown);
...
public:
    ...
    ~FileDialogImplHtml() override {
        dbp("FileDialogImplHtml::~FileSaveDialogImplHtml()");
...
class FileSaveDialogImplHtml : public FileDialog {
public:
    ...
    FileSaveDialogImplHtml() {
        dbp("FileSaveDialogImplHtml::FileSaveDialogImplHtml()");
        val fileManagerUIClass = val::global("window")["FileManagerUI"];
        val dialogModeValue = val(1); // FileManagerUI_SAVE
        this->jsFileManagerUI = fileManagerUIClass.new_(dialogModeValue);
    }

    ~FileSaveDialogImplHtml() override {
        dbp("FileSaveDialogImplHtml::~FileSaveDialogImplHtml()");
        this->jsFileManagerUI.call<void>("dispose");
    }

    void SetTitle(std::string title) override {
        dbp("FileSaveDialogImplHtml::SetTitle(): title=\"%s\"", title.c_str());
        this->title = title;
        this->jsFileManagerUI.call<void>("setTitle", val(title));
    }

    void SetCurrentName(std::string name) override {
        dbp("FileSaveDialogImplHtml::SetCurrentName(): name=\"%s\", parent=\"%s\"", name.c_str(), this->GetFilename().Parent().raw.c_str());
        SetFilename(GetFilename().Parent().Join(name));
    }
    ...
    void SetFilename(Platform::Path path) override {
        // NOTE: param "path" is fullpath.
        //       But we only treat filename.
        dbp("FileSaveDialogImplHtml::GetFilename(): path=\"%s\"", path.raw.c_str());
        this->filename = basePathInFilesystem + path.FileName();
        std::string filename_ = Path::From(this->filename).FileName();
        this->jsFileManagerUI.call<void>("setDefaultFilename", val(filename_.c_str()));
    }
    
    void SuggestFilename(Platform::Path path) override {
        dbp("FileSaveDialogImplHtml::SuggestFilename(): path=\"%s\"", path.raw.c_str());
        SetFilename(Platform::Path::From(path.FileStem()));
    }
    ...
    void AddFilter(std::string name, std::vector<std::string> extensions) override {
        ...
        dbp("FileSaveDialogImplHtml::AddFilter(): filter=%s", this->filters.c_str());
        this->jsFileManagerUI.call<void>("setFilter", val(this->filters));
    }
    ...
    bool RunModal() override {
        dbp("FileSaveDialogImplHtml::RunModal()");
